...
export const useMessageAndErrorOther = (
  ...
  useEffect(() => {
    ...
    if (message) {
      ...
      navigateTo && navigation.navigate(navigateTo);//this means if navigateTo exist then navigate else do nothing

      func && dispatch(func());//if func exist then dispatch else do nothing
    }
  }, [error, message, dispatch]);
...
export const useGetOrders = (isFocused, isAdmin = false) => {
  ...
  useEffect(() => {
    setLoading(true);
    axios
      .get(`${server}/order/${isAdmin ? "admin" : "myOrders"}`)
      .then((res) => {
